...
```
Tenga cuidado al utilizar operaciones lógicas de igualdad en R. En muchas ocasiones se utiliza = en vez de ==, es usual que este inconveniente suceda por lo tanto R les notificara con un mensaje de error
...
Las computadoras utilizan aritmética de operación finita, es decir, no se almacenan números con infinitos dígitos, por lo tanto, recuerde que cada número es una aproximación. En vez de usar ==, utilice la función near() que permite comparar dos vectores con números decimales: 
...
Una caracteristica importante de R que puede generar molestias en una comparación son los valores faltantes (missing value) o NAs (Not Availables).
